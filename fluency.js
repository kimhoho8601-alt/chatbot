...
const experienceQuestions=[
  {
    key:'tools',
    label:'업무에 사용해본 AI 도구는 몇 개인가요?',
    help:'예: ChatGPT, Claude, Gemini, Copilot, Perplexity, NotebookLM 등',
    options:[['사용해본 적 없음',0],['1개',1],['2개',2],['3~4개',3],['5개 이상',4]]
  },
  {
    key:'frequency',
    label:'최근 한 달 동안 업무에 AI를 얼마나 자주 사용했나요?',
    help:'개인적인 사용이 아니라 실제 업무 목적의 사용 기준입니다.',
    options:[['거의 사용하지 않음',0],['월 1~2회',1],['주 1~2회',2],['주 3~4회',3],['거의 매일',4]]
  },
  {
    key:'functions',
    label:'업무에서 사용해본 AI 활용 방식은 어느 정도인가요?',
    help:'문서 작성뿐 아니라 분석, 검색, 이미지, 자동화 등 활용 범위를 봅니다.',
    options:[['단순 질문·검색 정도',0],['문장 작성·요약',1],['자료 분석·아이디어 정리',2],['이미지·파일·데이터 등 여러 기능',3],['자동화·코딩·업무도구 연계까지',4]]
  },
  {
    key:'workflow',
    label:'AI를 업무 과정에 어느 정도까지 적용해봤나요?',
    help:'일회성 사용보다 실제 업무 흐름 안에서 활용한 경험을 봅니다.',
    options:[['시험 삼아 사용',0],['일부 과업에 가끔 사용',1],['반복 업무에 정기적으로 사용',2],['업무 단계 여러 곳에 활용',3],['업무 흐름 자체를 AI 중심으로 재설계',4]]
  },
  {
    key:'output',
    label:'AI 결과물을 실제 업무 산출물에 어느 정도 활용해봤나요?',
    help:'보고서, 자료, 분석, 안내문, 발표자료, 코드 등 실제 결과물 기준입니다.',
    options:[['거의 활용하지 않음',0],['참고 아이디어 정도',1],['수정 후 일부 반영',2],['주요 초안이나 분석에 활용',3],['검토를 거쳐 핵심 산출물에 정기 활용',4]]
  }
];
...
$('#beginBtn').onclick=()=>{
  ...
  experienceAnswers={};answers=Array(questions.length).fill(null);current=0;
...
$('#nextBtn').onclick=()=>{
  ...
  if(current<questions.length-1){current++;renderQuestion()}else submitResult();
};
$('#retakeBtn').onclick=()=>{
  $('#personalResult').classList.add('hidden');$('#namePanel').classList.remove('hidden');$('#quizCard').classList.add('hidden');$('#experienceCard').classList.add('hidden');experienceAnswers={};answers=Array(questions.length).fill(null);current=0;updateProgress('준비');$('#assessment').scrollIntoView({behavior:'smooth'});
};
...
function renderQuestion(){
  ...
  $('#scaleOptions').querySelectorAll('button').forEach(btn=>btn.onclick=()=>{
    ...
    $('#scaleOptions').querySelectorAll('button').forEach(option=>option.classList.toggle('active',option===btn));
...
async function submitResult(){
  ...
  renderPersonal(payload);$('#quizCard').classList.add('hidden');$('#personalResult').classList.remove('hidden');$('#personalResult').scrollIntoView({behavior:'smooth'});
  ...
  toast('결과가 TF 보드에 공유되었습니다.');await loadTeam();
}

function renderPersonal(r){
  $('#resultName').textContent=r.participant_name;$('#totalScore').textContent=r.total_score;$('#utilScore').textContent=r.utilization_score??0;$('#utilBar').style.width=`${r.utilization_score??0}%`;
...
function renderTeam(){
  ...
  if(!results.length){$('#teamTotalAvg').textContent='0';$('#teamUtilAvg').textContent='0';drawRadar($('#teamRadar'),{delegation:0,description:0,discernment:0,diligence:0},true);$('#teamBars').innerHTML='';$('#peopleGrid').innerHTML='<div class="empty-state">아직 제출된 결과가 없습니다.</div>';return}
  ...
  $('#teamTotalAvg').textContent=totalAvg;$('#teamUtilAvg').textContent=utilAvg;
